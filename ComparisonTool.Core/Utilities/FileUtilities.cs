...
public class FileUtilities : IFileUtilities
{
    ...
    private void AppendFolderComparisonPairDetails(StringBuilder sb, int pairNumber, FilePairComparisonResult result)
    {
        ...
        else
        {
            ...
            foreach (var category in (result.Summary?.DifferencesByChangeType ?? new Dictionary<DifferenceCategory, IList<Difference>>()).OrderByDescending(c => c.Value.Count).Take(5))
            {
