...
public class DirectoryComparisonService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var performanceScopeId = CreatePerformanceScopeId("DirectoryComparison");
        using var performanceScope = performanceTracker.BeginScope(performanceScopeId);
        var propertyIgnoreCacheBaseline = PropertyIgnoreHelper.GetCacheStats();
        var operationId = performanceTracker.StartOperation("CompareDirectoriesAsync");
        MultiFolderComparisonResult? finalResult = null;
        ComparisonPhaseTimingContext? phaseTimings = null;
        string sessionId = null;

        try
        {
            phaseTimings = new ComparisonPhaseTimingContext("Directory path comparison");
            ...
            try
            {
                ...
                if (filePairs.Count == 0)
                {
                    ...
                    var emptyResult = new MultiFolderComparisonResult
                    {
                        AllEqual = true,
                ...
                if (finalResult == null)
                {
                    ...
                    if (ShouldUseHighPerformancePipeline(filePairs.Count))
                    {
                        result = await CompareWithHighPerformancePipelineAsync(
                            filePairs,
                            modelName,
                            sessionId,
                            progress,
        ...
        throw new InvalidOperationException("Directory comparison completed without producing a result.");
    }
    ...
    public async Task<MultiFolderComparisonResult> CompareFolderUploadsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var performanceScopeId = CreatePerformanceScopeId("FolderComparison");
        using var performanceScope = performanceTracker.BeginScope(performanceScopeId);
        var propertyIgnoreCacheBaseline = PropertyIgnoreHelper.GetCacheStats();
        var operationId = performanceTracker.StartOperation("CompareFolderUploadsAsync");
        MultiFolderComparisonResult? finalResult = null;
        ComparisonPhaseTimingContext? phaseTimings = null;
        string sessionId = null;
        ...
        try
        {
            phaseTimings = new ComparisonPhaseTimingContext("Folder upload comparison");
            using var timingScope = ComparisonPhaseTimingScope.Push(phaseTimings);
            ...
            // Start comparison logging session
            sessionId = comparisonLogService.StartSession(modelName, pairCount);

            progress?.Report(new ComparisonProgress(0, pairCount, $"Will compare {pairCount} files in order"));

            MultiFolderComparisonResult result;
            if (ShouldUseHighPerformancePipeline(pairCount))
            {
                var orderedFilePairs = CreateOrderedFilePairs(folder1Files, folder2Files, pairCount);
                result = await CompareWithHighPerformancePipelineAsync(
                    orderedFilePairs,
                    modelName,
                    sessionId,
        ...
        throw new InvalidOperationException("Folder upload comparison completed without producing a result.");
    }

    private void FinalizeRunMetadata(
        MultiFolderComparisonResult result,
        ComparisonPhaseTimingContext phaseTimings,
        string reportPrefix,
        string performanceScopeId,
        (long hits, long misses, double hitRatio) propertyIgnoreCacheBaseline)
    {
    ...
    {
        logger.LogInformation(
            "Using high-performance pipeline for {Count} file pairs (threshold: {Threshold})",
            filePairs.Count,
            ComparisonOrchestrator.HighPerformancePipelineThreshold);

        var pipelineResult = await highPerformanceComparisonPipeline.CompareFilesAsync(
            filePairs
                .Select(filePair => (
                    File1Path: filePair.file1Path,
                    File2Path: filePair.file2Path,
                    RelativePath: filePair.relativePath))
                .ToList(),
            modelName,
            progress,
            cancellationToken).ConfigureAwait(false);

        EnsureComparisonSessionMetadata(pipelineResult, sessionId);
        ReplayComparisonResultsToSessionLog(sessionId, pipelineResult.FilePairResults);
    ...
    private async Task AnalyzeComparisonResultAsync(
        ...
        CancellationToken cancellationToken)
    {
        result.Metadata.TryGetValue("ComparisonSessionId", out var sessionIdValue);
        EnsureComparisonSessionMetadata(result, sessionIdValue as string);
    ...
    private List<(string file1Path, string file2Path, string relativePath)> CreateOrderedFilePairs(
        IReadOnlyList<string> folder1Files,
        IReadOnlyList<string> folder2Files,
        int pairCount) =>
        Enumerable.Range(0, pairCount)
            .Select(index => (
                file1Path: folder1Files[index],
                file2Path: folder2Files[index],
                relativePath: Path.GetFileName(folder1Files[index])))
            .ToList();

    private bool ShouldUseHighPerformancePipeline(int pairCount)
        => pairCount >= ComparisonOrchestrator.HighPerformancePipelineThreshold;

    private void EnsureComparisonSessionMetadata(MultiFolderComparisonResult result, string? sessionId)
    {
        result.Metadata ??= new Dictionary<string, object>(StringComparer.Ordinal);
    ...
    private void ReplayComparisonResultsToSessionLog(string sessionId, IEnumerable<FilePairComparisonResult> filePairResults)
    {
        foreach (var filePairResult in filePairResults)
        {
            comparisonLogService.LogFilePairResult(sessionId, filePairResult);
        }
    }

    private void AddPropertyIgnoreCacheMetadata(
        IDictionary<string, object> metadata,
        (long hits, long misses, double hitRatio) baseline)
    {
        var (currentHits, currentMisses, _) = PropertyIgnoreHelper.GetCacheStats();
        var hits = Math.Max(0, currentHits - baseline.hits);
        var misses = Math.Max(0, currentMisses - baseline.misses);
        var total = hits + misses;
        var hitRatio = total > 0 ? (double)hits / total : 0.0;

        metadata["PropertyIgnoreCacheHits"] = hits;
        metadata["PropertyIgnoreCacheMisses"] = misses;
        ...
        ComparisonPhaseTimings phaseTimings)
    {
        try
        {
            performanceTracker.LogReportForScope(performanceScopeId);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Failed to log performance report for scope {PerformanceScopeId}", performanceScopeId);
        }
        ...
        {
            logger.LogWarning(ex, "Failed to save performance text report for scope {PerformanceScopeId}", performanceScopeId);
        }
