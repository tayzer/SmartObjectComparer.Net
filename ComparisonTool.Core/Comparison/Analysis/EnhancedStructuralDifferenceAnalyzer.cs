...
public class EnhancedStructuralDifferenceAnalyzer
{
    ...
    private readonly HashSet<string> criticalProperties = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
        "Test",
    };
    ...
    private readonly HashSet<string> importantParentPaths = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    public EnhancedStructuralAnalysisResult AnalyzeStructuralPatterns()
    {
        ...
        var valueDifferences = new Dictionary<string, Dictionary<(string OldValue, string NewValue), int>>(StringComparer.Ordinal);
        ...
        logger?.LogInformation(
            "Enhanced structural analysis complete. Found {CriticalCount} critical missing elements, {MissingProps} consistently missing properties, {OrderDiffs} element order differences, and {UncategorizedCount} uncategorized differences",
            result.CriticalMissingElements.Count,
            result.ConsistentlyMissingProperties.Count,
            result.ElementOrderDifferences.Count,
            result.UncategorizedDifferences.Count);
        ...
        HashSet<Difference> categorizedDifferences)
    {
        var criticalDifferences = allDifferences
            ...
            .GroupBy(c => NormalizePropertyPath(c.Difference.PropertyName), StringComparer.Ordinal);
        ...
        foreach (var group in criticalDifferences)
        {
            ...
            if (segments.Count > 0)
            {
                ...
                {
                    var affectedFiles = group.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
    ...
    private void AnalyzeMissingProperties(List<(Difference difference, string FilePair, FilePairComparisonResult Result)> allDifferences, Dictionary<string, StructuralPattern> structuralPatterns, HashSet<Difference> categorizedDifferences)
    {
        ...
        {
            // Skip already categorized differences
            if (categorizedDifferences.Contains(diff))
            {
                continue;
            }
    ...
    private void AnalyzeValueDifferences(List<(Difference Difference, string FilePair, FilePairComparisonResult Result)> allDifferences, Dictionary<string, Dictionary<(string OldValue, string NewValue), int>> valueDifferences, Dictionary<string, StructuralPattern> structuralPatterns, HashSet<Difference> categorizedDifferences)
    {
        ...
        foreach (var pathEntry in valueDifferences)
        {
            ...
            if (mostCommonChange.Key != default && mostCommonChange.Value > 1)
            {
                ...
                // Find all UNCATEGORIZED differences matching this path and value change
                var matchingDiffs = allDifferences
                    .Where(d => !categorizedDifferences.Contains(d.Difference) &&
string.Equals(NormalizePropertyPath(d.Difference.PropertyName), path, StringComparison.Ordinal) &&
                                (string.Equals(d.Difference.Object1Value?.ToString(), oldValue, StringComparison.Ordinal) ||
                                 (d.Difference.Object1Value == null && string.Equals(oldValue, "null", StringComparison.Ordinal))) &&
                                (string.Equals(d.Difference.Object2Value?.ToString(), newValue, StringComparison.Ordinal) ||
                                 (d.Difference.Object2Value == null && string.Equals(newValue, "null", StringComparison.Ordinal))))
                    .ToList();
                ...
                if (matchingDiffs.Count > 1)
                {
                    ...
                    if (!structuralPatterns.ContainsKey(pattern))
                    {
                        var affectedFiles = matchingDiffs.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
    ...
    {
        var fileGroups = allDifferences.GroupBy(d => d.FilePair, StringComparer.Ordinal);

        foreach (var fileGroup in fileGroups)
        {
            var filePair = fileGroup.Key;

            // Filter to only uncategorized collection differences
    ...
    private void AnalyzeCollectionElements(List<(Difference Difference, string FilePair, FilePairComparisonResult Result)> allDifferences, Dictionary<string, StructuralPattern> structuralPatterns, HashSet<Difference> categorizedDifferences)
    {
        ...
        {
            // Skip already categorized differences
            if (categorizedDifferences.Contains(difference))
            {
                continue;
            }
    ...
    private DifferenceCategory DetermineValueChangeCategory(string oldValue, string newValue)
    {
        ...
        if ((string.Equals(oldValue, "True", StringComparison.Ordinal) || string.Equals(oldValue, "False", StringComparison.Ordinal)) && (string.Equals(newValue, "True", StringComparison.Ordinal) || string.Equals(newValue, "False", StringComparison.Ordinal)))
        {
    ...
    private string GenerateValueChangeDescription(string propertyName, string oldValue, string newValue)
    {
        ...
        if (string.IsNullOrEmpty(oldValue) || string.Equals(oldValue, "null", StringComparison.Ordinal))
        {
            return $"A new property '{lastSegment}' is now present";
        }

        // Handle value to null changes (property becoming absent)
        if (string.IsNullOrEmpty(newValue) || string.Equals(newValue, "null", StringComparison.Ordinal))
        {
    ...
    private void AnalyzeGeneralValueDifferences(List<(Difference Difference, string FilePair, FilePairComparisonResult Result)> allDifferences, Dictionary<string, StructuralPattern> structuralPatterns, HashSet<Difference> categorizedDifferences)
    {
        ...
        // ONLY for differences not already categorized by other analyzers
        var generalChangeCounts = new Dictionary<string, Dictionary<string, int>>(StringComparer.Ordinal);

        // First pass: count UNCATEGORIZED value changes by normalized path
        foreach (var (difference, filePair, _) in allDifferences)
        {
            // Only count non-missing value differences that are NOT already categorized
            if (!categorizedDifferences.Contains(difference) &&
                !IsPropertyMissing(difference) &&
                ...
                difference.Object2Value != null)
            {
                ...
                {
                    generalChangeCounts[normalizedPath] = new Dictionary<string, int>(StringComparer.Ordinal);
                }
        ...
        foreach (var pathEntry in generalChangeCounts)
        {
            ...
            // Skip if no uncategorized differences for this path
            if (fileCounts.Values.Sum() == 0)
            {
                continue;
            }
            ...
            if (!structuralPatterns.ContainsKey(pattern))
            {
                ...
                // Find all UNCATEGORIZED matching differences for this path
...
string.Equals(NormalizePropertyPath(d.Difference.PropertyName), path, StringComparison.Ordinal) &&
    ...
    private void AnalyzeUncategorizedDifferences(List<(Difference Difference, string FilePair, FilePairComparisonResult Result)> allDifferences, HashSet<Difference> categorizedDifferences, Dictionary<string, StructuralPattern> structuralPatterns)
    {
        ...
        if (uncategorizedDifferences.Count > 0)
        {
            ...
            if (!structuralPatterns.ContainsKey(pattern))
            {
                var affectedFiles = uncategorizedDifferences.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
    ...
    private void CalculateExclusiveFileCounts(EnhancedStructuralAnalysisResult result)
    {
        ...
        var fileCategoryMap = new Dictionary<string, HashSet<string>>(StringComparer.Ordinal);

        foreach (var file in allFilesWithDifferences)
        {
            fileCategoryMap[file] = new HashSet<string>(StringComparer.Ordinal);
        }

        // Determine which categories each file appears in
        foreach (var pattern in result.AllPatterns)
        {
            var categoryGroup = GetCategoryGroup(pattern.Category);
        ...
        var categoryCombinations = new Dictionary<string, int>(StringComparer.Ordinal);
        foreach (var file in fileCategoryMap.Where(f => f.Value.Count > 1))
        {
            var combination = string.Join(" + ", file.Value.OrderBy(c => c, StringComparer.Ordinal));
    ...
    private void CalculateValueSubcategoryBreakdowns(EnhancedStructuralAnalysisResult result)
    {
        ...
        var valueFiles = result.AllPatterns
            .Where(p => string.Equals(GetCategoryGroup(p.Category), "Value", StringComparison.Ordinal))
            .SelectMany(p => p.AffectedFiles)
            .Distinct(StringComparer.Ordinal)
            .ToHashSet(StringComparer.Ordinal);

        // Group value files by which value subcategories they appear in
        var valueFileCategoryMap = new Dictionary<string, HashSet<string>>(StringComparer.Ordinal);

        foreach (var file in valueFiles)
        {
            valueFileCategoryMap[file] = new HashSet<string>(StringComparer.Ordinal);
        }

        // Determine which value subcategories each file appears in
        foreach (var pattern in result.AllPatterns.Where(p => string.Equals(GetCategoryGroup(p.Category), "Value", StringComparison.Ordinal)))
        {
            var valueSubcategory = GetValueSubcategory(pattern.Category);
            foreach (var file in pattern.AffectedFiles)
            {
        ...
        var valueCategoryCombinations = new Dictionary<string, int>(StringComparer.Ordinal);
    ...
    private string ClassifyFilePrimaryDifferenceType(FilePairComparisonResult filePair)
    {
        ...
        if (valueCount > totalDifferences * 0.2)
        {
            significantCategories++;
        }

        if (missingCount > totalDifferences * 0.2)
        {
            significantCategories++;
        }

        if (orderCount > totalDifferences * 0.2)
        {
            significantCategories++;
        }
    ...
    {
        required public string ParentPath
        {
            get; set;
        }

        required public string MissingProperty
        {
            get; set;
        }

        required public string FullPattern
        {
            get; set;
        }
        ...
        public bool IsCriticalProperty
        {
            get; set;
        }

        required public string HumanReadableDescription
        {
            get; set;
        }

        required public string RecommendAction
        {
            get; set;
        }
    }
    ...
    public class FileCoverageAnalysis
    {
        ...
        /// </summary>
        public Dictionary<string, List<string>> FilesByCategory { get; set; } = new Dictionary<string, List<string>>(StringComparer.Ordinal);

        /// <summary>
        /// Gets or sets count of files in each category.
        /// </summary>
        public Dictionary<string, int> FileCounts { get; set; } = new Dictionary<string, int>(StringComparer.Ordinal);

        /// <summary>
        /// Gets or sets total files processed (should equal sum of all categories).
        /// </summary>
        ...
        /// <summary>
        /// Gets a value indicating whether validation: does the sum of categories equal total files?.
        /// </summary>
    ...
    public class EnhancedStructuralAnalysisResult
    {
        ...
        /// <summary>
        /// Gets or sets differences that don't fit into any specific pattern category.
        /// </summary>
        ...
        public int TotalDifferencesFound
        {
            get; set;
        }

        public int CriticalDifferencesFound
        {
            get; set;
        }

        public int UncategorizedDifferencesFound
        {
            get; set;
        }
        ...
        /// <summary>
        /// Gets or sets list of file pairs with differences that are not covered by any pattern (for diagnostics/UI).
        /// </summary>
        public List<string> UnaccountedFilesWithDifferences { get; set; } = new List<string>();

        /// <summary>
        /// Gets count of unaccounted files with differences.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets files that have ONLY value differences (no other categories).
        /// </summary>
        public int ExclusiveValueDifferenceFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY missing property differences (no other categories).
        /// </summary>
        public int ExclusiveMissingPropertyFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY order differences (no other categories).
        /// </summary>
        public int ExclusiveOrderDifferenceFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY uncategorized differences (no other categories).
        /// </summary>
        public int ExclusiveUncategorizedFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that appear in multiple categories.
        /// </summary>
        public int MultiCategoryFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets breakdown of which combinations of categories appear together.
        /// </summary>
        public Dictionary<string, int> CategoryCombinations { get; set; } = new Dictionary<string, int>(StringComparer.Ordinal);

        // Value difference subcategory breakdowns

        /// <summary>
        /// Gets or sets files that have ONLY text content value differences.
        /// </summary>
        public int ExclusiveTextContentFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY numeric value differences.
        /// </summary>
        public int ExclusiveNumericValueFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY boolean value differences.
        /// </summary>
        public int ExclusiveBooleanValueFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY date/time value differences.
        /// </summary>
        public int ExclusiveDateTimeFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that have ONLY general value differences.
        /// </summary>
        public int ExclusiveGeneralValueFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets files that appear in multiple value difference subcategories.
        /// </summary>
        public int MultiValueCategoryFiles
        {
            get; set;
        }

        /// <summary>
        /// Gets or sets dictionary mapping value category combinations to file counts.
        /// </summary>
        public Dictionary<string, int> ValueCategoryCombinations { get; set; } = new Dictionary<string, int>(StringComparer.Ordinal);

        /// <summary>
        /// Gets or sets file-first classification that ensures complete coverage.
        /// </summary>
