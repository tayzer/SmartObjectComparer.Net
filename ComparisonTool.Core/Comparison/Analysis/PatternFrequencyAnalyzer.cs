    ...
    public List<PatternFrequencyGroup> Analyze(IEnumerable<Difference> allDifferences, Dictionary<Difference, string> differencesToFilePairMap, int totalFiles)
    {
        var groups = allDifferences
            ...
            .Select(g => new PatternFrequencyGroup
            {
                ...
                FileCount = g.Select(d => differencesToFilePairMap.ContainsKey(d) ? differencesToFilePairMap[d] : "unknown").Distinct(StringComparer.Ordinal).Count(),
                AffectedFiles = g.Select(d => differencesToFilePairMap.ContainsKey(d) ? differencesToFilePairMap[d] : "unknown").Distinct(StringComparer.Ordinal).ToList(),
