...
public class MinHash
{
    ...
    public MinHash(int numHashes = 64)
    {
        ...
        hashSeeds = Enumerable.Range(0, numHashes).Select(_ => rand.Next()).ToArray();
    }

    public int[] ComputeSignature(IEnumerable<string> set)
    {
        var signature = new int[numHashes];
        ...
        foreach (var item in set)
        {
            ...
            {
                var hash = item.GetHashCode() ^ hashSeeds[i];
                if (hash < signature[i])
                {
                    signature[i] = hash;
                }
