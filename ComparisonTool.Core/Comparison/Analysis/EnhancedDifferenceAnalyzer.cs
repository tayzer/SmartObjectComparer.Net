...
public class EnhancedDifferenceAnalyzer
{
    ...
    private readonly Dictionary<string, Regex> fieldPatterns = new Dictionary<string, Regex>(StringComparer.Ordinal)
    {
    ...
    public EnhancedAnalysisResult AnalyzeWithStructuralPatterns()
    {
        ...
        var xmlPathOccurrences = new Dictionary<string, (int Count, HashSet<string> Files)>(StringComparer.Ordinal);
        ...
        foreach (var filePair in folderResult.FilePairResults)
        {
            ...
            foreach (var diff in filePair.Result?.Differences ?? new System.Collections.Generic.List<KellermanSoftware.CompareNetObjects.Difference>())
            {
                ...
                {
                    xmlPathOccurrences[normalizedPath] = (0, new HashSet<string>(StringComparer.Ordinal));
                }
        ...
        Dictionary<string, StructuralPattern> patterns)
    {
        ...
        var collectionDiffs = differences
            ...
            .GroupBy(d => ExtractCollectionPath(d.Diff.PropertyName), StringComparer.Ordinal);
        ...
        foreach (var collectionGroup in collectionDiffs)
        {
            ...
            var missingProps = collectionGroup
                ...
                .GroupBy(d => ExtractPropertyAfterIndex(d.Diff.PropertyName), StringComparer.Ordinal);

            foreach (var propGroup in missingProps)
            {
                // Only interested in recurring patterns
                if (propGroup.Count() > 1)
                {
                    ...
                    {
                        var filesAffected = propGroup.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
            ...
            // Arbitrary threshold to suggest a potentially significant collection
            if (elementIndices.Count > 10)
            {
                var patternKey = $"{collectionPath}_count";
                if (!patterns.ContainsKey(patternKey))
                {
                    var filesAffected = collectionGroup.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
                    patterns[patternKey] = new StructuralPattern
                    {
        ...
        Dictionary<string, StructuralPattern> patterns)
    {
        ...
        var nonCollectionDiffs = differences
            .Where(d => !d.Diff.PropertyName.Contains("[") && d.Diff.PropertyName.Contains("."))
            .GroupBy(d => ExtractParentPath(d.Diff.PropertyName), StringComparer.Ordinal);
        ...
        foreach (var parentGroup in nonCollectionDiffs)
        {
            ...
            var missingProps = parentGroup
                .Where(d => IsPropertyMissing(d.Diff))
                .GroupBy(d => ExtractLastProperty(d.Diff.PropertyName), StringComparer.Ordinal);
            ...
            foreach (var propGroup in missingProps)
            {
                ...
                if (propGroup.Count() > 1)
                {
                    ...
                    var category = string.Equals(fieldType, "Identifier", StringComparison.Ordinal) ?
                    ...
                    if (!patterns.ContainsKey(patternKey))
                    {
                        ...
                        patterns[patternKey] = new StructuralPattern
                        {
                            ...
                            PotentialRootCause = string.Equals(fieldType, "Identifier", StringComparison.Ordinal) ?
        ...
        Dictionary<string, StructuralPattern> patterns)
    {
        ...
        var pathGroups = differences
            .GroupBy(d => NormalizeXmlPath(d.Diff.PropertyName), StringComparer.Ordinal);
        ...
        foreach (var pathGroup in pathGroups)
        {
            ...
            if (typeMismatches.Count > 0)
            {
                ...
                if (!patterns.ContainsKey(patternKey))
                {
                    var filesAffected = typeMismatches.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
            ...
            if (pathGroup.Any(d => IsExtraProperty(d.Diff)))
            {
                ...
                {
                    var filesAffected = extraProps.Select(d => d.FilePair).Distinct(StringComparer.Ordinal).ToList();
                    patterns[patternKey] = new StructuralPattern
                    {
                        XmlPath = normalizedPath,
    ...
    public class EnhancedAnalysisResult
    {
        ...
        public List<StructuralPattern> RecurringMissingElements { get; set; } = new List<StructuralPattern>();

        // Properties with inconsistent presence
        public List<StructuralPattern> InconsistentProperties { get; set; } = new List<StructuralPattern>();

        // Structural issues in the XML
        public List<StructuralPattern> StructuralIssues { get; set; } = new List<StructuralPattern>();

        // Differences by enhanced category
        public Dictionary<EnhancedDifferenceCategory, List<Difference>> DifferencesByCategory { get; set; } = new Dictionary<EnhancedDifferenceCategory, List<Difference>>();

        // Differences by XML path pattern
        public Dictionary<string, List<Difference>> DifferencesByXmlPath { get; set; } = new Dictionary<string, List<Difference>>(StringComparer.Ordinal);

        // Statistics by category
        public Dictionary<EnhancedDifferenceCategory, int> CategoryCounts { get; set; } = new Dictionary<EnhancedDifferenceCategory, int>();

        // Patterns with high impact (affect multiple files consistently)
        public List<StructuralPattern> HighImpactPatterns { get; set; } = new List<StructuralPattern>();
        ...
        public int FilePairsWithDifferences
        {
            get; set;
        }
    }
    ...
    public class StructuralPattern
    {
        ...
        public EnhancedDifferenceCategory Category
        {
            get; set;
        }
        ...
        public bool IsCollectionPattern
        {
            get; set;
        }
