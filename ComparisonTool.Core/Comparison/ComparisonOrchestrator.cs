...
public class ComparisonOrchestrator : IComparisonOrchestrator
{
        ...
        CancellationToken cancellationToken = default)
    {
        return await _performanceTracker.TrackOperationAsync("CompareXmlFilesWithCaching", async () =>
        {
            try
            {
                ...
                var result = await _comparisonEngine.CompareObjectsAsync(oldResponse, newResponse, modelType, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await _performanceTracker.TrackOperationAsync("CompareFilesAsync", async () =>
        {
            try
            {
                ...
                // Use the comparison engine for the actual comparison
                return await _comparisonEngine.CompareObjectsAsync(oldResponse, newResponse, modelType, cancellationToken);
            }
            catch (Exception ex)
            {
    ...
    private int CalculateOptimalBatchSize(int fileCount, IEnumerable<string> sampleFiles = null)
    {
        ...
        int baseBatchSize = Math.Max(10, Math.Min(50, fileCount / 4));
        ...
        int optimalParallelism = Math.Max(1, Math.Min(cpuCores - 1, 4));
        
        // Adjust batch size based on parallelism
        int adjustedBatchSize = baseBatchSize * optimalParallelism;
        
        // Cap at reasonable limits
        adjustedBatchSize = Math.Min(adjustedBatchSize, 100);
        adjustedBatchSize = Math.Max(adjustedBatchSize, 10);
        
        logger.LogDebug("Calculated optimal batch size: {BatchSize} (files: {FileCount}, cores: {Cores}, memory: {Memory}GB)",
    ...
    private int CalculateOptimalParallelism(int fileCount, IEnumerable<string> sampleFiles = null)
    {
        ...
        logger.LogDebug("Calculated optimal parallelism: {Parallelism} (files: {FileCount}, cores: {Cores})",
