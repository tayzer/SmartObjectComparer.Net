...
// A plugin package is self-contained: it carries its own model types rather than
// referencing the product. These mirror the source SOAP request/response, the
// endpoint B JSON request/response, and the canonical comparison type.
...
/// <summary>The bearer token endpoint B requires, from the chained token exchange.</summary>
